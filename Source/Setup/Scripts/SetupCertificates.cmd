...
%cscript% /nologo %capicompath% import -l LM "%~dp0certs\identityTkCloudRp.pfx" "STSTest"
%cscript% /nologo %capicompath% import -l CU "%~dp0certs\identityTkCloudRp.pfx" "STSTest"
%cscript% /nologo %capicompath% import -l CU -s trustedpeople "%~dp0certs\identityTkCloudRp.pfx" "STSTest"
...
@if "%IsW7%" == "true" (
	...
	"%~dp0winhttpcertcfg.exe" -g -c LOCAL_MACHINE\My -s identitytk_cloud_rp.cloudapp.net -a "IIS_IUSRS"
	...
	"%~dp0winhttpcertcfg.exe" -g -c LOCAL_MACHINE\My -s identitytk_cloud_rp.cloudapp.net -a "NETWORK SERVICE"
)
